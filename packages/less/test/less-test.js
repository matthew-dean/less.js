...
module.exports = function() {
    ...
    function testSourcemap(
        ...
        var expectedSourceMapURL = name + ".css.map",
            ...
            sourceMappingSuffix = " */",
            expectedCSSAppendage =
                sourceMappingPrefix +
                expectedSourceMapURL +
                sourceMappingSuffix;
        if (!compiledLess.endsWith(expectedCSSAppendage)) {
            ...
            var indexOfSourceMappingPrefix = compiledLess.indexOf(
                sourceMappingPrefix
            );
            if (indexOfSourceMappingPrefix === -1) {
                fail(
                    "ERROR: sourceMappingURL was not found in " +
                        baseFolder +
                        "/" +
                        name +
                        ".css."
            ...
            var startOfSourceMappingValue =
                    indexOfSourceMappingPrefix + sourceMappingPrefix.length,
                indexOfNextSpace = compiledLess.indexOf(
                    " ",
                    startOfSourceMappingValue
                ),
                actualSourceMapURL = compiledLess.substring(
                    startOfSourceMappingValue,
                    indexOfNextSpace === -1
                        ? compiledLess.length
                        : indexOfNextSpace
            ...
            fail(
                'ERROR: sourceMappingURL should be "' +
                    expectedSourceMapURL +
                    '" but is "' +
                    actualSourceMapURL +
                    '".'
        ...
        fs.readFile(path.join("test/", name) + ".json", "utf8", function(
            e,
            expectedSourcemap
        ) {
    ...
    function testErrors(
        ...
        fs.readFile(path.join(baseFolder, name) + ".txt", "utf8", function(
            e,
            expectedErr
        ) {
            process.stdout.write("- " + path.join(baseFolder, name) + ": ");
            expectedErr = doReplacements(
                expectedErr,
                baseFolder,
                err && err.filename
            );
    ...
    function globalReplacements(input, directory, filename) {
        ...
        return input
            ...
            .replace(
                /\{pathrel\}/g,
                path.join(path.relative(process.cwd(), p), "/")
            )
    ...
    function runTestSetInternal(
        ...
        fs.readdirSync(path.join(baseFolder, foldername)).forEach(function(
            file
        ) {
            if (!/\.less$/.test(file)) {
                return;
            ...
            if (options.sourceMap && !options.sourceMap.sourceMapFileInline) {
                ...
                options.sourceMap.sourceMapFilename =
                    options.sourceMap.sourceMapBasepath +
                    "/" +
                    options.sourceMap.sourceMapOutputFilename +
                    ".map";
            }
                ...
                if (item.added || item.removed) {
                    var text =
                        ...
                        item.value
                            ...
                            .replace("\ufeff", "[[BOM]]");
                    process.stdout.write(
                        stylize(text, item.added ? "green" : "red")
                    );
                } else {
                    process.stdout.write(
    ...
    function endTest() {
        if (isFinished && failedTests + passedTests >= totalTests) {
            ...
            if (leaked.length > 0) {
                process.stdout.write("\n");
                process.stdout.write(
                    stylize("Global leak detected: ", "red") +
                        leaked.join(", ") +
                        "\n"
                );
            }
    ...
    function toCSS(options, path, callback) {
        ...
        if (options.plugin) {
            var Plugin = require(require("path").resolve(
                process.cwd(),
                options.plugin
            ));
    ...
    function testNoOptions() {
        ...
        try {
            process.stdout.write(
                "- Integration - creating parser without options: "
            );
            less.render("");
        } catch (e) {
            fail(stylize("FAIL\n", "red"));
