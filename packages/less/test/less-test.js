...
module.exports = function() {
        ...
        if (!compiledLess.endsWith(expectedCSSAppendage)) {
            ...
            var startOfSourceMappingValue = indexOfSourceMappingPrefix + sourceMappingPrefix.length,
                ...
                actualSourceMapURL = compiledLess.substring(
                    ...
                    indexOfNextSpace === -1 ? compiledLess.length : indexOfNextSpace
                )
            fail(
    ...
    function testJSImport() {
        ...
        toCSS({}, require('path').join(process.cwd(), 'test/less/root-registry/root.less'), function(
            error,
            output
        ) {
            if (error) {
        ...
        fs.readdirSync(path.join(baseFolder, foldername)).forEach(function(file) {
          ...
          '.map'
            ...
            options.getVars = function(file) {
                try {
                    return JSON.parse(
                        fs.readFileSync(getFilename(getBasename(file), 'vars', baseFolder), 'utf8')
                    )
                } catch (e) {
                    return {}
                }
