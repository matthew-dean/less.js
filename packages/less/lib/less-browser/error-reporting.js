...
export default (window, less, options) => {
    function errorHTML(e, rootHref) {
        ...
        content =
            `<h3>${e.type || "Syntax"}Error: ${e.message ||
        ...
        if (e.line) {
            ...
            errorline(e, 1, "line");
            errorline(e, 2, "");
            content += `on line ${e.line}, column ${e.column +
                1}:</p><ul>${errors.join("")}</ul>`;
        }
    ...
    function errorConsole(e, rootHref) {
        ...
        const filename = e.filename || rootHref;
        const errors = [];
        let content = `${e.type || "Syntax"}Error: ${e.message ||
            "There is an error in your .less file"} in ${filename}`;
