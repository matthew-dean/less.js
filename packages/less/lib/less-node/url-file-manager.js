        ...
        return new Promise((fulfill, reject) => {
            ...
            if (!request) {
                reject({
                    type: "File",
                    message:
                        "optional dependency 'request' required to import over http(s)\n"
                });
                return;
            }
            ...
            request.get(
                { uri: urlStr, strictSSL: !options.insecure },
                (error, res, body) => {
                    if (error) {
                        reject({
                            type: "File",
                    ...
                    if (!body) {
                        logger.warn(
                            `Warning: Empty body (HTTP ${res.statusCode}) returned by "${urlStr}"`
                        );
                    }
