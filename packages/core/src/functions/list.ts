...
export const _SELF = define(function(n: Node) { return n }, [Node])
export const extract = define(
  function(value: Node, index: Num) {
    // (1-based index)
    let i = index.value - 1
...
export const range = define(
  function(start: Num | Dimension, end: Num | Dimension, step: Num) {
    let from: number
    let to: Node
    let stepValue = 1
    const list = []
...
export const each = define(
  function(list: Node, rs: Mixin) {
    const rules = []
    let newRules
    let iterator
