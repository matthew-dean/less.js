...
class LessParser extends CstParser {
    ...
    args = this.RULE("args", (inMixin: true) => {
        this.OR([
            {
                GATE: () =>
                    this._searchUntil([T.RParen, T.SemiColon]) === T.SemiColon,
                ALT: () => {
            ...
            {
                ALT: () =>
                    this.SUBRULE(this.expressionList, { ARGS: [inMixin] })
            }
        ]);
    });
            ...
            { ALT: () => this.SUBRULE(this.mediaNot) },
            {
                ALT: () => {
                    ...
                    this.OR2([
                        {
                            ALT: () =>
                                this.MANY(() => this.SUBRULE(this.mediaAnd))
                        },
                        {
                            ALT: () =>
                                this.MANY2(() => this.SUBRULE(this.mediaOr))
                        }
                    ]);
                }
            }
